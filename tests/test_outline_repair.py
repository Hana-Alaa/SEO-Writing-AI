...
class TestOutlineRepairService(unittest.TestCase):
    ...
    def test_enrich_brand_utility_faq_positive(self):
        ...
        repaired = self.repair_service.enrich_brand_utility_faq(outline, serp_brief, "Official brand: BrandX.", "informational")
        self.assertEqual(len(repaired[0]["subheadings"]), 2)
        self.assertEqual(repaired[0]["subheadings"][1]["heading_text"], "How to book via BrandX")

    def test_enrich_brand_utility_faq_duplicate(self):
        outline = [{"section_type": "faq", "heading_text": "FAQ", "subheadings": ["How to use BrandX?"]}]
        serp_brief = {"brand_utility_candidates": ["How to book via BrandX"]}
        repaired = self.repair_service.enrich_brand_utility_faq(outline, serp_brief, "Official brand: BrandX.", "informational")
    ...
    def test_enrich_brand_utility_faq_commercial_rejected(self):
        outline = [{"section_type": "faq", "heading_text": "FAQ", "subheadings": ["Q1"]}]
        serp_brief = {"brand_utility_candidates": ["How to book via BrandX"]}
        repaired = self.repair_service.enrich_brand_utility_faq(outline, serp_brief, "Official brand: BrandX.", "commercial")
        self.assertEqual(len(repaired[0]["subheadings"]), 1)

    def test_enrich_brand_utility_faq_replaces_weak_when_full(self):
        outline = [{"section_type": "faq", "heading_text": "FAQ", "subheadings": ["price 1", "ticket 2", "book 3", "location 4", "general weak question"]}]
        serp_brief = {"brand_utility_candidates": ["How to book via BrandX"]}
        repaired = self.repair_service.enrich_brand_utility_faq(outline, serp_brief, "Official brand: BrandX.", "informational")
        self.assertEqual(len(repaired[0]["subheadings"]), 5)
        # The last one should be replaced since it's weak
        self.assertEqual(repaired[0]["subheadings"][4]["heading_text"], "How to book via BrandX")

    def test_enrich_brand_utility_faq_skips_when_full_and_strong(self):
        outline = [{"section_type": "faq", "heading_text": "FAQ", "subheadings": ["price 1", "ticket 2", "book 3", "location 4", "hour 5"]}]
        serp_brief = {"brand_utility_candidates": ["How to book via BrandX"]}
        repaired = self.repair_service.enrich_brand_utility_faq(outline, serp_brief, "Official brand: BrandX.", "informational")
        self.assertEqual(len(repaired[0]["subheadings"]), 5)
        # Should not insert since all are strong
        self.assertEqual(repaired[0]["subheadings"][4], "hour 5")
