...
class AsyncWorkflowController:
    ...
    async def _discover_logo_and_colors(self, url: str, state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logo_local_path = os.path.join(output_dir, "assets/images", logo_filename)
    ...
    def validate_outline(self, outline, article_type):
        structure = self.REQUIRED_STRUCTURE_BY_TYPE.get(article_type, {})
        required = structure.get("mandatory", set())
        existing = {section.get("section_type") for section in outline}
    ...
    #     primary_keyword = state["primary_keyword"]
    #     with open("assets/prompts/templates/seo_semantic_layer.txt") as f:
    #         template = Template(f.read())
